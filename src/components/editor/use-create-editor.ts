...
export const useCreateEditor = () => {
  const { content, setContent } = useGlobalContext();
  ...
  useEffect(() => {
    if (editor?.api?.markdown) {
      try {
        ...
        // Split by headers, ensuring we don't lose content
        ...
        sections.forEach((section, index) => {
          ...
          const potentialContent = currentSection + section;
          
          // Create new page if max length exceeded or last section
          if (potentialContent.length >= MAX_LENGTH || index === sections.length - 1) {
            if (currentSection) {
              pagesArray.push({
        ...
        // Set initial page content if pages exist and no current content
        if (pagesArray.length > 0 && !editorContent) {
          setEditorContent(pagesArray[0].content);
        }
  ...
  }, [editor?.children, setContent]);

  // Update editor when page changes
  useEffect(() => {
    const currentPageData = pages.find(p => p.pageNumber === currentPage);
    if (currentPageData && editor) {
      try {
        const newContent = deserializeMd(editor, currentPageData.content, {
          processor(processor) {
            return processor.use(remarkEmoji) as any;
          },
        });
        ...
        editor.onChange();
        setEditorContent(currentPageData.content);
      } catch (error) {
